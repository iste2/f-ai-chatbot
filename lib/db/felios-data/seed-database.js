...
const shiftModels = [
  { name: 'Frühschicht', daily_capacity: 8 },
  { name: 'Spätschicht', daily_capacity: 8 },
  { name: 'Nachtschicht', daily_capacity: 8 },
  { name: 'Abwesend', daily_capacity: 0 }
];
...
// Insert shift models
const shiftIds = shiftModels.map(s => db.prepare('INSERT INTO shift (name, daily_capacity) VALUES (?, ?)').run(s.name, s.daily_capacity).lastInsertRowid);
...
for (const opId of allOperationIds) {
  ...
  const empId = qualified[randomInt(0, qualified.length - 1)].employee_id;
  ...
  for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
    ...
    db.prepare('INSERT INTO operation_assignment (operation_id, employee_id, date, assigned_capacity) VALUES (?, ?, ?, ?)')
      .run(opId, empId, d.toISOString().slice(0, 10), op.time_capacity_demand);
  }
}
