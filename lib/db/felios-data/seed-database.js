...
const shiftModels = [
  { name: 'Frühschicht', daily_capacity: 8, color_code: '#FFD600' }, // Bright Yellow (Morning)
  { name: 'Spätschicht', daily_capacity: 8, color_code: '#FF7043' }, // Deep Orange (Evening)
  { name: 'Nachtschicht', daily_capacity: 8, color_code: '#283593' }, // Indigo (Night)
  { name: 'Abwesend', daily_capacity: 0, color_code: '#BDBDBD' } // Neutral Grey (Absent)
];
...
for (let p = 0; p < 50; p++) {
  ...
  for (let n = 0; n < numNetworks; n++) {
    ...
    for (let o = 0; o < numOps; o++) {
      ...
      // Set time_capacity_demand to 5-10x duration
      const timeCapacity = randomInt(5 * duration, 10 * duration);
